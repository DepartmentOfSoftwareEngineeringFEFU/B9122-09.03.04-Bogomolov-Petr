...
Кнопка **🚪 Выйти** есть в меню каждой роли: отвязывает Telegram ID от учётной
записи и возвращает на экран первичной привязки (с показом текущего
Telegram ID для повторной привязки через веб-интерфейс или API).
...
| `/teacher/lessons/<id>/attendance/` | Преподаватель | Отметить посещаемость |
...
с эвристиками MCV/LCV и учитывает при генерации все ограничения C1–C7:
- C1: преподаватель не может вести два занятия одновременно
- C2: класс не может быть на двух занятиях одновременно
- C3: корректность временного интервала (начало раньше конца)
- C4: преподаватель назначается только на дисциплину, которую он ведёт
- C5: недельная нагрузка преподавателя не превышает `max_hours_per_week`
- C6: покрытие учебного плана (часы по каждой дисциплине для класса)
- C7: два класса с одинаковой аудиторией по умолчанию не пересекаются по времени

Два режима генерации: **полная** (все занятия удаляются и создаются заново) и
**дозаполнение** (существующие занятия сохраняются, достраиваются только
недостающие по учебному плану). При невозможности разместить часть занятий
система выводит причину для каждого нерешённого занятия.
